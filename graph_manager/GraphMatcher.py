...
class GraphMatcher():
    ...
    def match_custom(self, G1_name, G2_name):
        ...
        def match_iteration(working_node_ID, lvl):
            ...
            if working_node_ID:
                # parents_data = {"match" : working_node_attrs["match"], "parent1" : working_node_attrs["data_parent1"], "parent2" : working_node_attrs["data_parent2"], "id" : working_node_ID}
                ...
                data1, data2, all_pairs_and_parent_numerical = self.add_parents_data(data1, data2, all_pairs_numerical, working_node_attrs["data_parent1"],working_node_attrs["data_parent2"])
                data1 = self.geometric_info_transformation(data1, sweeped_levels[lvl], working_node_attrs["data_parent1"])
                data2 = self.geometric_info_transformation(data2, sweeped_levels[lvl], working_node_attrs["data_parent2"])
            ...
            for A_categorical in matches:
                ...
                self.logger.info("clipper_match_numerical, score {} {}".format(clipper_match_numerical, score))
            ...
            if filter1_scores:
                ...
                for good_submatch_i in sorted_matches_indexes:
                    ...
                    if lvl < len(sweeped_levels) - 1:
                        parent1_data = self.change_pos_dt(G1_full, np.array(list(filter1_matches[good_submatch_i]))[:,0], sweeped_levels_dt[sweeped_levels[lvl]], sweeped_levels_dt[sweeped_levels[lvl+1]])
                        parent2_data = self.change_pos_dt(G2_full, np.array(list(filter1_matches[good_submatch_i]))[:,1], sweeped_levels_dt[sweeped_levels[lvl]], sweeped_levels_dt[sweeped_levels[lvl+1]])
                        for i, lowerlevel_match_pair in enumerate(filter1_matches[good_submatch_i]):               
                            ...
                            if not existing_nodes:
                                ...
                                node_attr = [(pair_node_id, {"type": sweeped_levels[lvl], "match": lowerlevel_match_pair,\
                                                            ...
                                                            "data_parent1" : parent1_data[i], "data_parent2" : parent2_data[i]})]
        ...
        final_matches_msg = self.full_graph_affinity_filter(self.graphs[G1_name], self.graphs[G2_name], sweeped_levels,\
    ...
    def full_graph_affinity_filter(self, G1, G2, sweeped_levels, sweeped_levels_dt, sweeped_levels_ci, matches_tuples_list):
