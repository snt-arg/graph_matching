...
postprocess = [{"pp_name": "dropout", "room": 0.5, "ws":0.0}]
...
for i in range(len(s_dataset)):
    ...
    # visualize_nxgraph_3d(as_dataset[0], "as_graph", visualize_alone=True, include_node_ids=True)


    ### CREATE GRAPH MATCHER

    graph_matcher = GraphMatcher(fake_logger, log_level = 5 )
    ...
    if success:
        ...
        if len(matches) > 0:
            for i, match in enumerate(matches):
                ...
                visualize_nxgraph_3d(aux_as_graph, f"as_graph {i}", visualize_alone=True, include_node_ids=True)
